...
function solution(arr) {
    ...
    let numPassportValid = 0;
    let passport;
    for (let i = 0; i < arr.length; i++) {
        passport = arr[i].trim();
        //console.log("passport=" + passport);
        splittedPassport = passport.split(" ");
        if (isValidPassport(splittedPassport)) {
            numPassportValid++;
        }
    }
    return numPassportValid;
}

function isValidPassport1(passport) {
    let isValidPassport = true;

    if (passport.length >= 8) isValidPassport = true;
    else if (passport.length <= 6) isValidPassport = false;
    else {
        for (let i = 0; i < passport.length; i++) {
            let passportAttribute = passport[i];
            splittedPassportAttriute = passportAttribute.split(":");
            if (splittedPassportAttriute[0] == 'cid') {
...
function isValidPassport(passport) {
    let isValidPassport = true;
    if (passport.length <= 6) isValidPassport = false;
    else if (passport.length == 7 && contains(passport, 'cid')) isValidPassport = false;
    else {
        for (let i = 0; i < passport.length; i++) {
            let passportAttribute = passport[i];
            splittedPassportAttribute = passportAttribute.split(":");
            isValidPassport = isValidPassport && isValidPassportAttribute(splittedPassportAttribute[0], splittedPassportAttribute[1]);
        }
    }
    return isValidPassport;
}

function isValidPassportAttribute(name, value) {
    let isValidPassportAttribute = false;
    if (name === 'byr') {
        let numericValue = Number(value);
        isValidPassportAttribute = numericValue >= 1920 && numericValue <= 2002;
    } else if (name === 'iyr') {
        let numericValue = Number(value);
        isValidPassportAttribute = numericValue >= 2010 && numericValue <= 2020;
    } else if (name === 'eyr') {
        let numericValue = Number(value);
        isValidPassportAttribute = numericValue >= 2020 && numericValue <= 2030;
    } else if (name === 'hgt') {
        let numericHgt = value.substr(0, value.length - 2);
        let typeHgt = value.substr(value.length - 2, 2);
        if (typeHgt === 'in') {
            isValidPassportAttribute = numericHgt >= 59 && numericHgt <= 76;
        } else if (typeHgt === 'cm') {
            isValidPassportAttribute = numericHgt >= 150 && numericHgt <= 193;
        }
    } else if (name === 'hcl') {
        isValidPassportAttribute = value[0] == '#' && value.length == 7 && isValidHcl(value.substr(1, 6));
    } else if (name === 'ecl'){
        isValidPassportAttribute = isValidEcl(value);
    } else if (name === 'pid' ){
        isValidPassportAttribute = isValidPid(value);
    } else if (name === 'cid' ){
        isValidPassportAttribute = true;
    }
    //console.log("Passport attribute " + name + " with value " + value + " is " + isValidPassportAttribute);
    return isValidPassportAttribute;
}
...
function isValidEcl(ecl){
    let re = new RegExp("amb|blu|brn|gry|grn|hzl|oth");
    return re.test(ecl);
}

function isValidHcl(hcl) {
    let re = new RegExp("\\d");
    let re2 = new RegExp('[a-f]');
    return hcl.split("").every(element => {
        return re.test(element) || re2.test(element);
    });
}
function contains(arr, str) {
    for (let i = 0; i < arr.length; i++) {
        let passportAttribute = arr[i];
        splittedPassportAttribute = passportAttribute.split(":");
        if (splittedPassportAttribute[0] == str) {
            return true;
        }
    }
    return false;
...
console.log(solution(parseInputString()));

module.exports = isValidPid;
