let inputstr = `1-9 x: xwjgxtmrzxzmkx
4-6 r: rrrkrgr
4-5 v: vvfvvvn
...
let numOk=0;
 for(let i=0;i<arr.length;i++){
     ...
     let policy = ruleSplit[0];
     let password = ruleSplit[1].trim();
     let policySplit = policy.split(" ");
     let policyTimesLetter = policySplit[0];
     let policyLetter = policySplit[1];
     let minTimesLetter=parseInt(policyTimesLetter.split("-")[0]);
     let maxTimesLetter=parseInt(policyTimesLetter.split("-")[1]);
    if(solution(policyLetter,minTimesLetter,maxTimesLetter,password)){
        numOk++;
    }
 }
 return numOk;
}
function solution(policyLetter,minTimesLetter,maxTimesLetter,password){
    let arrPassword = password.split('');
    if(arrPassword.length<maxTimesLetter) {
        return arrPassword[minTimesLetter-1] == policyLetter;
    }else{
    return arrPassword[minTimesLetter-1] == policyLetter? 
            arrPassword[maxTimesLetter-1] != policyLetter :
            arrPassword[maxTimesLetter-1] == policyLetter 
            ;
        }
   }
   function solution1(policyLetter,minTimesLetter,maxTimesLetter,password){
    let arrPassword = password.split('');
    let cont = arrPassword.filter(l => l===policyLetter).length;
    return cont>=minTimesLetter && cont<= maxTimesLetter;
   }

module.exports = solution;

console.log("solution is "+solutionLoad());