...
let arrayOfRules = rules.split('\n');
let nameAndContainsStringSeparation = ' bags contain ';

class BagRule {

    constructor(str, name, containers) {
        this.textRule = str;
        this.name = name;
    ...
    isFinal() { return this.containers.length == 0 }
}
...
function extractName(str) {
    let i = str.trim().indexOf(nameAndContainsStringSeparation);
    return str.trim().substr(0, i);
}

function extractContains(str) {
    let i = str.indexOf(nameAndContainsStringSeparation);
    let j = nameAndContainsStringSeparation.length;
    let l = str.length - i - j;
    let containsString = str.substr(i + j, l);
    if (containsString[containsString.length - 1] === '.') {
        containsString = containsString.replace('.', '');
    }
    if (containsString.indexOf('no other bags') > -1)
        return [];
    else
        return parseToBagContainers(containsString);
}
function parseToBagContainers(containsString) {
    let arrayOfContainsString = containsString.split(', ');
    let containsArray = arrayOfContainsString.map(el => {
        let pos = el.trim().indexOf(' ');
        let num = parseInt(el.trim().substr(0, pos));
        let posBagString = el.indexOf('bag');
        let nom = el.substr(pos + 1, posBagString - 2).trim();
        return new BagContainer(num, nom);
    });
    return containsArray;
}

function parseToBagRules(arr) {
    let bagRules = arr.map(el => {
        return new BagRule(el, extractName(el), extractContains(el));
    });
    return bagRules;
}

function bagCanContainBag(mainBag, targetBag, arrayOfBags) {
    if (mainBag.getContainers().length == 0)
        return false;
    else
        return containersCanContainBag(mainBag.getContainers(), targetBag, arrayOfBags);
}
function countTotalNumOfBags(bag, arrayOfBags) {
    if (bag.getContainers().length == 0)
        return 1;
    else
        return 1 + countNumOfBagsInside(bag.getContainers(), arrayOfBags);
}
function countNumOfBagsInside(containers,  arrayOfBags) {
    let numsOfBagsInside = 0;
    for(let i =0; i< containers.length;i++)
    {
        let container = containers[i];
        let bag = findBagByName(arrayOfBags,container.getName());
        if(bag===undefined){
            console.log("Not found bag with name ["+container.getName()+"]");
        }
        numsOfBagsInside += container.getNum()*countTotalNumOfBags(bag,arrayOfBags);
    }
    return numsOfBagsInside;
}
function containersCanContainBag(containers, targetBag, arrayOfBags) {
    return containers.filter(el => {
        if (el.getName() === targetBag.getName()) return true;
        else
            return containerCanContainBag(el, targetBag, arrayOfBags);

    }).length > 0;
}
function containerCanContainBag(bagContainer, targetBag, arrayOfBagRules) {
    //console.log("finding bag with container.name "+bagContainer.getName());
    let bagContained = findBagByName(arrayOfBagRules,bagContainer.getName());
    if (bagContained !== undefined) {
        //console.log("finded bag with container.name "+bagContained.getName());
        return bagCanContainBag(bagContained,targetBag,arrayOfBagRules);
    }
}
//console.log(arrayOfRules);


exports.parseToBagRules = parseToBagRules;
exports.arrayOfRules = arrayOfRules;
exports.BagRule = BagRule;
exports.bagCanContainBag = bagCanContainBag;
exports.countNumOfBagsInside = countNumOfBagsInside;


function solution1(){
    let numOfBagsFinded=0;
    let arrayOfBagRules = parseToBagRules(inputRules.split('\n'));
    
    let targetBag = findBagByName(arrayOfBagsRules,'shiny gold');

    arrayOfBagRules.forEach(element => {
        if(bagCanContainBag(element,targetBag,arrayOfBagRules)){
            numOfBagsFinded++;
        }
    });
    return numOfBagsFinded;
}

function findBagByName(arrayOfBagRules,name){
    return  arrayOfBagRules.find(el => {
...
function solution(){
    ...
    let arrayOfBagRules = parseToBagRules(inputRules.split('\n'));
    let targetBag = findBagByName(arrayOfBagRules,'shiny gold');
    
    let numOfBagsFinded=countNumOfBagsInside(targetBag.getContainers(),arrayOfBagRules);
