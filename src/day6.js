...
function splitInGroups() {
    let splittedInput = testStr.split("\n");
    let groups = [];
    let currentGroup = [];
    for (let i = 0; i < splittedInput.length; i++) {
        let element = splittedInput[i];
        if (element != '' && element != '\n') {
            currentGroup = addRepeatedAnswersToGroup(element, currentGroup);
        } else {
...
function countRepeatedAnswers(group) {
    //array where each position are answers from one person
    let answers = group.shift().split('');
    for (let i = 0; i < group.length && answers.length>0; i++) {
        answers  = keepOnlyRepeatedAnswers(answers,group[i])      
    }
    return answers.length;
}

function keepOnlyRepeatedAnswers(currentAnswers,newAnswers){
    let b = newAnswers.split('');
    return currentAnswers.filter(answer=>b.some(el=>el===answer));
}

function addDifferentAnswersToGroup(answer, currentGroup) {
    let answers = answer.split('');
    answers.forEach(el => {
        if (!currentGroup.some(e => e === el))
            currentGroup.push(el);
    });
}

function addRepeatedAnswersToGroup(answer, currentGroup) {
    let answers = answer.split('');
    if (currentGroup.length === 0) {
        answers.forEach(el => currentGroup.push(el));
    } else {
        currentGroup = currentGroup.filter(el =>
            answers.some(e => e === el)
        );
    }
    return currentGroup;
...
function solution1() {
    let groups = splitInGroups();
    let answers = [];
    for (let i = 0; i < groups.length; i++) {
        const element = groups[i];
        let groupAnswer = element.length;
        answers.push(groupAnswer);
    }
    return answers.reduce((ac, el) => ac += el);
}

function solution(str){
    let groups = splitGroups(str);
    return groups.reduce((ac, el) => ac + countRepeatedAnswers(el),0);
}

//console.log('solution is '+solution(inputstr));
//console.log(splitInGroups());
//console.log(splitGroups(testStr));


exports.solution = solution;
exports.splitGroups = splitGroups;
exports.addRepeatedAnswersToGroup = addRepeatedAnswersToGroup;
exports.countRepeatedAnswers = countRepeatedAnswers;